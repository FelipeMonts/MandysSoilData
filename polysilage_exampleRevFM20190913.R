...
# or just set the working directory with the following instruction... note the the "\" backslash that windous uses needs to be changed to the forwardslash "/" that R and unix systems use

setwd("C:/Felipe/Students Projects/Mandy's Project/RCode")


#This line of code installs the Random Forest package. 
install.packages("randomForest")

#Once tha package is installed it is necesary to call the package so it is available in to use in your code. This is achived with the instruction library()
...
#Here, we need to define which columns of data will be considered factors. Factors are ones that are not a continuous variable--they are descrete, or cathegorical. Examples of factors are: color, type, Female-Male, etc. It is similar to putting the $ in SAS. 
...
f2<-read.table("polysoil.predict.rev.txt",sep = "", header = TRUE)
str(f2)

# don't forget to to define which columns of data will be considered factors
...
# there is a bug in the randomForest code that requires the levels of the factors of both prediction and traing data set to be consistent. 
# For example the levels of f1$year are "2016" "2017" "2018"

levels(f1$year)

# while he levels of f2$year are "2016" "2017"

levels(f2$year)



#To make surre the levels of both match there are two ways:
#the first one: include the training data with the new data you want to predict so all the levels of the factors in the training data are included for sure
#The second one is to expand the levels of the factors you want to prdict to match the one in the training set

levels(f2$year)<-c("2016" ,"2017", "2018")


#You readlly only need this line to run the prediction in Random Forest.  
